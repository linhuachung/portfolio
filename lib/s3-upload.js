 ...
 * @param {string} contentType - Content type (e.g., 'application/pdf', 'image/jpeg')
 * @param {string} folder - Folder in S3 bucket (default: 'resume')
 * @returns {Promise<{path: string, fileName: string, originalFileName: string}>}
 */
export async function uploadFileToS3( fileBuffer, fileName, originalFileName, contentType = 'application/pdf', folder = null ) {
  // Validate buffer
  ...
  const s3Folder = folder || process.env.AWS_S3_FOLDER || 'resume';
