 ...
 */
export function formatContactInfoForDisplay( contactInfo, defaults = {} ) {
  const defaultEmail = defaults.email || 'chunglh1304@gmail.com';
  const defaultAddress = defaults.address || 'Ho Chi Minh City';
  const defaultMapLink = defaults.addressMapLink || 'https://maps.app.goo.gl/tLqFHdNkU8eeYoXx6';

  return {
    phone: contactInfo.phone || '',
    email: contactInfo.email || defaultEmail,
    address: contactInfo.address || defaultAddress,
    emailLink: contactInfo.email ? `mailto:${contactInfo.email}` : `mailto:${defaultEmail}`,
    addressLink: defaultMapLink
  };
}
