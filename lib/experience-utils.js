...
export function extractTechName( tech ) {
  if ( typeof tech === 'string' ) {
    return tech;
  }
  return tech?.name || tech?.techStack?.name || String( tech );
}

export function getTechKey( tech, index ) {
  if ( tech?.id ) {
    return tech.id;
  }
  if ( tech?.techStack?.id ) {
    return tech.techStack.id;
  }
  const techName = extractTechName( tech );
  return `${techName}-${index}`;
}
